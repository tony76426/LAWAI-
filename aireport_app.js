...
一、案件類型判斷
請依使用者描述之實際事件，判斷最可能的案件類型（以一般民眾可理解的用語表達，不要寫成過度抽象的學科分類）。

二、情境選項生成（極重要：要「架構化」，不是改寫）
請產生 3 個「情境選項」，三個選項必須是「同一事件」但用不同法律聚焦架構來整理，讓使用者能一眼看出自己要走哪種方向：
A.【契約／約定與履行】（例如：有無契約、約定內容、對方是否未履行）
B.【告知／不實表示／隱瞞】（例如：對方是否有說法、是否有刻意隱瞞或誤導）
C.【損害／證據／目標與程序】（例如：造成什麼損失、有哪些證據、目前卡點與想要的結果）

每一個選項都必須同時符合：
1) 保留使用者輸入裡至少 2 個具體事實元素（人物身分、行為、爭議物、金額、地點、目前狀態等），且至少有 1 個關鍵名詞或片語需與使用者原始輸入「完全同字出現」，不得只用概括詞替代（如僅寫成「糾紛」「問題」）。
2) 不得加入使用者未提及的新情節、角色或行為；不得假設、延伸或重構成典型案例；不得更換爭議主體、主要行為或權利義務對象。
3) 三個選項要「看起來差異明顯」：聚焦點與待確認重點要不同，不可只是換句話說或只改幾個字。
4) 每個選項請用「第一人稱敘述句」寫成一段完整敘事，字數 80～100 字，且盡量接近 100 字。
5) 每個選項末尾必須加上一段「待確認：…」列出 2～3 個關鍵待確認點（用短詞即可，例如：是否有書面約定／是否已通知對方／是否有付款或對話紀錄）。注意：待確認點只能是「需要補充的資訊」，不得把它寫成已經發生的事實。
6) 不得出現具體的年月日或精確時間（例如「2023 年 5 月」「晚上 8 點」），如有需要僅可使用「之前」「近期」「這段時間」等相對描述。

三、禁止事項
- 不得使用「常見情況」「通常」「一般來說」等概括語
- 不得改變事件主體或時間順序
- 不得加入責任歸屬結論或法律評價（例如：已違法、一定可以告、一定成立）
